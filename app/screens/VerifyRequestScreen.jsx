...
const VerifyRequestScreen = ({ navigation }) => {
  ...
  const handleResendRequest = async () => {
    ...
    setLoading(true);
    try {
      const result = await requestVerificationToken(email);
      setLoading(false);
      if (result.success) {
  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: COLORS.white }}>
      ...
      >
        <ScrollView contentContainerStyle={{ flexGrow: 1, paddingHorizontal: SIZES.padding }}>
          <View style={{ marginTop: 40, flexDirection: 'row', alignItems: 'center' }}>
            ...
            <View>
              <Text style={{ fontSize: SIZES.h1, fontWeight: '800', color: COLORS.text }}>Verify Account</Text>
              <Text style={{ fontSize: SIZES.body, color: COLORS.gray, marginTop: 5 }}>Request another verification link</Text>
            </View>
          </View>
            ...
            >
              ...
              >
                {loading ? (
                  ...
                  <Text style={{ color: COLORS.white, fontSize: 18, fontWeight: "bold" }}>Send Verification Link</Text>
                )}
              </LinearGradient>
            </TouchableOpacity>
          </KeyboardAvoidingView>
      ...
      <InfoModal 
        ...
        title="Email Sent"
        message="A verification link has been sent to your email. Please check your inbox and spam folder."
        icon="check-circle"
